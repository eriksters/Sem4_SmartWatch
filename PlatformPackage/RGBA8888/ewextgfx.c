  ...
  void* aDisplay1, void* aDisplay2, void* aDisplay3 )
{
  ...
  viewport->FrameBuffer->BytesPerPixel = sizeof( long );
  ...
  viewport->FrameBuffer->Clut          = 0;
    ...
    viewport->DoubleBuffer->BytesPerPixel = sizeof( long );
...
unsigned long GfxCreateSurface( int aFormat, int aWidth, int aHeight )
{
  ...
  {
    bytesPerPixel = sizeof( char );
    clutSize = 256 * sizeof( long ) + 3; /* clut startaddress has to be DWORD aligned */
  }
  ...
  if ( surface )
  {
    ...
    if ( clutSize > 0 )
      surface->Clut        = (void*)(((unsigned long)( surface + 1 ) + bitmapSize + 3 ) & 0xFFFFFFFC );
    else
      surface->Clut        = 0;
