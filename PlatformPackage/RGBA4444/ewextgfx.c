  ...
  void* aDisplay1, void* aDisplay2, void* aDisplay3 )
{
  ...
  viewport->FrameBuffer->Clut          = 0;

  #ifdef EW_USE_GRAPHICS_ACCELERATOR
    viewport->FrameBuffer->AccelClut   = 0;
  #endif
    ...
    viewport->DoubleBuffer->Clut          = 0;

    #ifdef EW_USE_GRAPHICS_ACCELERATOR
      viewport->DoubleBuffer->AccelClut   = 0;
    #endif
...
unsigned long GfxCreateSurface( int aFormat, int aWidth, int aHeight )
{
  ...
  {
    bytesPerPixel = sizeof( char );
    clutSize = 256 * sizeof( long );

    #ifdef EW_USE_GRAPHICS_ACCELERATOR
      accelClutSize = 256 * sizeof( long ) + 3; /* clut startaddress has to be DWORD aligned */
    #endif
  }
  ...
  if ( surface )
  {
    ...
    if ( clutSize > 0 )
      ...
      if ( accelClutSize > 0 )
        surface->AccelClut   = (void*)(((unsigned long)( surface + 1 ) + bitmapSize + clutSize + 3 ) & 0xFFFFFFFC );
      else
        surface->AccelClut   = 0;
    #endif
  ...
  if ( bytesPerPixel > 0 )
  {
    ...
    if ( surface )
    {
      ...
      surface->Clut          = (void*)( aMemory->Clut );
        ...
        if ( accelClutSize > 0 )
        {
          surface->AccelClut   = (void*)(((unsigned long)( surface + 1 ) + 3 ) & 0xFFFFFFFC );
